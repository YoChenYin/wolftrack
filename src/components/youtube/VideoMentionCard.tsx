import { findYoutubeChannel } from "@/config/youtubeChannels";
import { StockMentionBadge } from "./StockMentionBadge";
import type { VideoMentionSummary } from "@/lib/youtube/queries";
import { SubCard } from "../ui/Card";

export function VideoMentionCard({ video }: { video: VideoMentionSummary }) {
  const channel = findYoutubeChannel(video.channelId);

  return (
    <SubCard>
      <div className="flex items-baseline justify-between gap-2">
        <span className="text-xs font-medium text-zinc-500 dark:text-zinc-400">
          {channel?.displayName ?? video.channelId}
        </span>
        <span className="text-[11px] text-zinc-400 dark:text-zinc-500">{video.publishedAt.slice(0, 10)}</span>
      </div>
      <p className="mt-1 text-sm font-medium text-zinc-800 dark:text-zinc-200">{video.title}</p>
      {video.summary && <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">{video.summary}</p>}
      {video.keySignals.length > 0 && (
        <ul className="mt-1.5 list-disc space-y-0.5 pl-4 text-xs text-zinc-600 dark:text-zinc-400">
          {video.keySignals.map((signal, i) => (
            <li key={i}>{signal}</li>
          ))}
        </ul>
      )}
      {video.mentions.length > 0 && (
        <div className="mt-2 flex flex-wrap gap-1.5">
          {video.mentions.map((mention) => (
            <StockMentionBadge key={mention.id} mention={mention} />
          ))}
        </div>
      )}
    </SubCard>
  );
}
