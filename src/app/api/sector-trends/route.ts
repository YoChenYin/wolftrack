...
/**
 * GET /api/sector-trends?market={US|TW}&sector={sectorCode}&theme={themeCode}&mode={reversal|pullback|bullish}&limit={n}
 *
...
export async function GET(request: NextRequest) {
  ...
  if (modeParam && !TREND_STATUSES.includes(modeParam as TacticalStatus)) {
    return NextResponse.json(
      { error: `Invalid mode "${modeParam}". Must be one of: ${TREND_STATUSES.join(", ")}` },
      { status: 400 }
    );
  }
