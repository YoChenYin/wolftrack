...
**結論**：回測的程式邏輯/管線是對的（已用小樣本測過、事件數量合理、跟正式分類邏輯共用同一套函式不會有規則不一致的問題），但**要跑在prod資料庫**（median 2358個交易日≈9-10年連續歷史）才能拿到有意義的驗證結果。新增`/api/cron/tw-signal-backtest`（手動觸發，`POST` + `Authorization: Bearer $CRON_SECRET`，不掛進每日排程——這是「調整參數後想重新驗證」才需要手動跑一次的離線分析，不是每天要抓的資料）。已跟使用者說明這個發現，等使用者決定要不要現在對prod跑一次拿真實答案。UI（拿真實數字取代「效果未驗證」badge）先不做——本機這批數字方向不可信，貿然接上UI會顯示誤導性的「已驗證」結果。
