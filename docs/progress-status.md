...
### 2.27 美股策略研究：ML預測 vs 純技術分析規則（2026-08-28~29）

使用者提出的獨立研究題目——以MU/NVDA為起點，探索美股交易策略。完整過程/方法論/最終結論記錄在專門的文件：[`docs/us-quant-strategy-research.md`](us-quant-strategy-research.md)，這裡只記重點：

- ML模型（gradient boosting，價格/技術特徵，含K棒/OBV/頭肩底型態特徵）在單股、20檔同業合併訓練、擴充特徵集三種setup下都沒有找到真實預測能力，一路輸給「順勢延續」這種最簡單的基準。
- 改測純技術分析規則（黃金/死亡交叉、RSI超賣超買、MACD轉正轉負、頭肩底/N字底），美股半導體20檔2年歷史裡「RSI超賣反彈」看起來很強（60日超額報酬+11.6%），但擴大到台股9-10年多市場狀態驗證、修正掉股票池本身的存活者偏差（能被完整回填長歷史的股票本來就系統性跑贏大盤）後，**這個訊號反轉成穩定的負訊號**——證實只是牛市regime巧合。
- 唯一存活的訊號：**頭肩底/N字底確認**，修正後依然穩定為正且隨持有期間增強（60日邊際超額報酬+6.39%）。這個型態偵測邏輯（`detectBottomPattern.ts`）已經是這個repo既有功能，這次算是給它補上一份「歷史上這個訊號到底準不準」的量化證據。
- 新增`TwTechnicalRuleBacktestEvent`表 + `backtestTechnicalRules.ts`（正式併入既有回測基礎建設，local dev DB上跑，不需要prod資料庫）；美股那邊的Python研究腳本留在`scripts/ml-research/`供之後參考，沒有併入正式app。

尚未排入實作：把頭肩底/N字底的真實勝率呈現到台股個股頁/每日異動報告UI上。
...
- 新增`/tw/etf`分頁（`TwSectionNav`第六個分頁），獨立整理413檔ETF。
...
**驗證**：本機dev server確認`/tw/etf`正確顯示分類數字（跟直接query DB結果一致），11檔有資料的ETF（例如00400A主動國泰動能高息，最新收盤13.41）正確渲染；`/tw`主選股頁確認仍正常運作。`tsc --noEmit`/`eslint`乾淨。

**2026-08-29分類邏輯改版**：使用者提出「高股息/高成長」這種行銷用語式分類不夠直接，改用**股票代號後綴字母**當主分類——這是TWSE/TPEx官方編碼慣例（L=槓桿、R=反向、U=期貨商品、A=主動式、B=債券型、無後綴=一般型），反映的是風險機制而不是行銷話術，資料本身（代號字串）已經有了不用額外資料源。次要標籤用基金名稱關鍵字比對（高股息/科技成長/ESG/中小型），抓得到才標。分類結果：一般型190/債券型135/主動式29/槓桿型26/反向型26/期貨商品型7，加總413檔。

**新增「今日領漲類型」**：使用者提出「從每日漲跌幅排序可以看出當日領漲股票類型」——`computeLeadingTypes()`依結構型分類算平均日漲跌幅、由高到低排序，樣本數<3檔的類型不列入（避免單一檔ETF的漲跌被誤讀成整個類型的動向）。每個結構型分類的表格內部也改成依日漲跌幅排序（有資料的排前面、漲最多的排最上面），不再是單純照代號排序。

**已知限制**：413檔裡只有11檔有價格歷史，樣本太少時「今日領漲類型」的意義有限（本機驗證時只有1-2個類型湊到3檔門檻）；要讓這個功能真正有用，需要另外跑ETF的歷史回填（`tw-backfill.ts`理論上可以指定ticker清單重跑，但413檔全部回填預估要幾小時，這次沒有執行）；代號後綴分類保守處理了不確定語意的字母（例如K），統一歸「一般型」，沒有猜測。
